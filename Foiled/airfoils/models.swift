...
//enum Orientation { case clockwise, counterclockwise }
...
struct Contour: Decodable, Shape {
    ...
    init(from decoder: Decoder) throws {
        ...
        dot = 0.005
        options = []
        //edge = coordinates[0].x.isZero ? .leading : .trailing
    }
    
    /*
    func camber() {
        let sorted = coordinates.sorted(by: { $0.x < $1.x } )
        guard let middle = edge == .leading ? sorted.first : sorted.last else { return }
    }
    
    mutating func starts(with edge: Edge, orientation: Orientation) {
        guard edge != self.edge else { return }
        let sorted = coordinates.sorted(by: { $0.x < $1.x } )
        guard let middle = edge == .leading ? sorted.first : sorted.last else { return }
        guard let index = coordinates.firstIndex(of: middle) else { return }
    }
     */
}
